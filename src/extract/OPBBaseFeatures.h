...
public:
  TermSum(StreamBuffer &in) {
    for (in.skipWhitespace(); *in != ';' && *in != '>' && *in != '=';
         in.skipWhitespace()) {
      std::string coeffstr;
      ...
      int var;
      in.readInteger(&var);
      if (Var(var + 1) > max_var)
        max_var = Var(var + 1);
      coeffs.push_back(coeff);
    }
  }

  inline size_t nTerms() { return coeffs.size(); }

  inline double const minVal() { return min; }

  inline double const maxVal() { return max; }

  inline Var const maxVar() { return max_var; }

  inline double const minCoeff() { return abs_min_coeff; }
};
...
public:
  ...
  inline Analysis analyse() {
    ...
    case GE:
      ...
      a.assignment =
          terms.maxVal() - terms.minCoeff() < bound && terms.maxVal() > bound;
      a.clause =
          bound > terms.minVal() && bound <= terms.minVal() + terms.minCoeff();
      break;
    case EQ:
    default:
  ...
  inline Var maxVar() { return terms.maxVar(); }
};
...
public:
  BaseFeatures(const char *filename)
      : filename_(filename), features(), names() {
    ...
    names.insert(names.end(),
                 {"obj_coeffs_min", "obj_coeffs_max", "obj_coeffs_entropy"});
  }
  ...
  virtual void extract() {
    ...
    while (in.skipWhitespace()) {
      if (*in == '*') {
        ...
        Constr constr(in);
        if (constr.maxVar() > n_vars)
          n_vars = constr.maxVar();
  ...
  void load_feature_record() {
    ...
    features.insert(features.end(), {(double)obj_terms, (double)obj_max_val,
                                     (double)obj_min_val});
    push_distribution(features, obj_coeffs);
  }

  virtual std::vector<double> getFeatures() const { return features; }
...
class BaseFeatures : public IExtractor {
  ...
  std::vector<std::vector<double>> obj_coeffs{};
...
public:
  BaseFeatures(const char *filename)
    ...
    names.insert(names.end(), {"pbs_ge", "pbs_eq", "cards_ge", "cards_eq"});
    names.insert(names.end(), {"clauses", "assignments", "trivially_unsat"});
    std::vector<std::string> o_features = {
        "terms",           "max_val",    "min_val",    "coeffs_mean",
        "coeffs_variance", "coeffs_min", "coeffs_max", "coeffs_entropy"};
  ...
  void load_feature_record() {
    ...
    features.insert(features.end(), {(double)n_clauses, (double)n_assignments});
    features.insert(features.end(), {(double)trivially_unsat});
    for (unsigned oidx = 0; oidx < std::min(n_objectives, N_OBJ_ANALYZED);
         oidx++) {
      features.insert(features.end(),
                      {(double)obj_terms[oidx], (double)obj_max_val[oidx],
                       (double)obj_min_val[oidx]});
